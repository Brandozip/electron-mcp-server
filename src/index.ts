...
const server = new Server(
  ...
  },
  {
    capabilities: {
      ...
      tools: {}, // Enable tools to satisfy client requirements
    },
  }
);
...
// Added Tool Handlers for Compatibility and Functionality
    ...
    tools: [
      {
        ...
        inputSchema: {
          ...
          properties: {
            appPath: {
              type: "string",
              description: "Full path to the Electron application folder or executable"
            },
            debugPort: {
              type: "number",
              description: "Optional port for remote debugging (default: random between 9222-9999)"
            }
          },
          required: ["appPath"]
        }
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            processId: {
              type: "string",
              description: "The ID of the process to stop (e.g., electron-12345678)"
            }
          },
          required: ["processId"]
        }
      ...
      {
        name: "list_electron_apps",
        description: "List all currently managed Electron applications",
        inputSchema: {
          type: "object",
          properties: {}
        }
      },
      {
        ...
        description: "List available CDP debug targets for a process",
        inputSchema: {
          type: "object",
          properties: {
            processId: {
              type: "string",
              description: "The ID of the process (e.g., electron-12345678)"
            }
          },
          required: ["processId"]
        }
      },
      {
        ...
        description: "Execute a Chrome DevTools Protocol command",
        inputSchema: {
          type: "object",
          properties: {
            processId: {
            ...
            targetId: {
              type: "string",
              description: "The ID of the CDP target (page/renderer)"
            },
            domain: {
              type: "string",
              description: "CDP Domain (e.g., Runtime, Page, Network)"
            },
            method: {
              type: "string",
              description: "CDP Method (e.g., evaluate, reload, navigate)"
            },
            params: {
              type: "object",
              description: "Arguments for the CDP command"
            }
          },
          required: ["processId", "targetId", "domain", "method"]
        }
      }
    ]
  };
  ...
  try {
    switch (name) {
      case "start_electron_app": {
        const { appPath, debugPort } = args as { appPath: string; debugPort?: number };
        const proc = await startElectronApp(appPath, debugPort);
        return {
          content: [
            {
              type: "text",
              text: `Started app: ${proc.name} (ID: ${proc.id}) on port ${proc.debugPort}`
            }
          ]
        };
      }
      ...
      case "stop_electron_app": {
        ...
        const success = stopElectronApp(processId);
        return {
          content: [
            {
              type: "text",
              text: success ? `Stopped process: ${processId}` : `Process not found: ${processId}`
            }
          ],
          isError: !success
        };
      ...
      case "list_electron_apps": {
        const apps = Array.from(electronProcesses.entries()).map(([id, proc]) => ({
          id,
          name: proc.name,
          status: proc.status,
          pid: proc.pid,
      ...
      case "list_debug_targets": {
        const { processId } = args as { processId: string };
        const process = electronProcesses.get(processId);
        if (!process) throw new Error(`Process ${processId} not found`);

        const targets = await updateCDPTargets(process);
        return {
          content: [
            {
              type: "text",
      ...
      case "execute_cdp_command": {
        const { processId, targetId, domain, method, params } = args as {
          processId: string;
          targetId: string;
          domain: string;
          method: string;
        ...
        const result = await executeCDPCommand(process, targetId, domain, method, params || {});
        return {
          content: [
            {
              type: "text",
...
console.info('{"jsonrpc": "2.0", "method": "log", "params": { "message": "Electron Debug MCP Server running (v1.1.0 with Tools)..." }}');